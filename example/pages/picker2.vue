<template>
  <div class="picker-render-view">
    ...
    <picker2 @select="handleSelect(0,arguments)" :selected-index="selectedIndex[0]"
            ref="picker0" :title="title[0]" :cancelTxt="$tc('pickerPage.cancelTxt', 1)"
            :confirmTxt="$tc('pickerPage.confirmTxt', 1)"></picker2>
    <div class="select" @click="showPicker(1)" ref="select1">{{ selectedText[1] }}</div>
    <picker2 @select="handleSelect(1,arguments)" :data="data[1]" :selected-index="selectedIndex[1]"
            ref="picker1" :title="title[1]" :cancelTxt="$tc('pickerPage.cancelTxt', 2)"
            :confirmTxt="$tc('pickerPage.confirmTxt', 2)"></picker2>

    <div class="select" @click="showPicker(2)" ref="select2">{{ selectedText[2] }}</div>
    <picker2 @select="handleSelect(2,arguments)" :data="data[2]" :selected-index="selectedIndex[2]"
